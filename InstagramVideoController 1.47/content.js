(function () {
    ...
    function hideVideoClickOverlay(overlay) {
        ...
        overlay.dataset.instagramVideoControllerInfoMoved = 'true';
    ...
    function createDebugPanel() {
        ...
        const steps = [
            ...
            ['7. Find overlay', debugFindOverlay],
            ['8. Restore overlay', debugRestoreOverlay],
            ['9. Click more', debugClickMore],
            ['10. Find legacy info', debugFindLegacyInfo],
            ['11. Find info by more', debugFindInfoByMore],
            ['12. Move info', debugMoveInfo],
            ['13. Hide overlay', debugHideOverlay],
            ['Run full sequence', debugRunFullSequence],
            ['Cleanup side box', () => {
                cleanupSideBox();
                debugAnchor = null;
                debugOverlay = null;
    ...
    function debugFindOverlay() {
        const video = requireActiveVideo();
        if (!video) return;
        debugOverlay = video.nextElementSibling;
    ...
    function debugFindLegacyInfo() {
        const video = requireActiveVideo();
        if (!video) return;
        debugInfoElement = findLegacyInfoElement(video);
        debugLog('legacy info element', debugInfoElement || 'none');
    }
    ...
    function debugMoveInfo() {
        ...
        if (!debugOverlay) debugOverlay = video.nextElementSibling;
        restoreVideoClickOverlayForInfoSearch(debugOverlay);
        clickMoreButton(debugOverlay);
        debugInfoElement = findLegacyInfoElement(video) || findInfoElementByMoreButton(video);
        if (!debugInfoElement) {
    ...
    function debugRunFullSequence() {
        ...
        if (debugInfoElement && debugInfoElement.parentElement === sideBoxInfo) {
            ...
            debugLog('skip hiding overlay because info was not moved');
        }
        updatePanel();
        debugLog('full sequence complete');
    }
