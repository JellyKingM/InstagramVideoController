(function () {
    ...
    function createPanel() {
        ...
        row3.appendChild(createButton('Hide box', 'Hide the side box', hideSideBox));
    ...
    function updatePanel() {
        ...
        statusEl.textContent = activeVideo
            ? `Videos: ${videos.length} | ${activeVideo.paused ? 'Paused' : 'Playing'} | ${options.volumeMute ? 'Muted' : 'Unmuted'} | Vol ${Math.round(options.volumeSliderV * 100)}% | ${options.playbackRateV.toFixed(2)}x | Side box ${options.sideBoxVisibleV ? 'Shown' : 'Hidden'}`
    ...
    function updateSideBoxRestoreButton(video) {
        ...
        if (!sideBoxRestoreButton) {
            sideBoxRestoreButton = document.createElement('button');
            sideBoxRestoreButton.id = 'instagram-video-controller-show-box';
            sideBoxRestoreButton.type = 'button';
            sideBoxRestoreButton.textContent = 'Show box';
            sideBoxRestoreButton.title = 'Show the side box';
            sideBoxRestoreButton.style.cssText = `
                ...
                box-shadow: 0 6px 18px rgba(0,0,0,0.35);
            `;
            sideBoxRestoreButton.addEventListener('click', event => {
                event.preventDefault();
                event.stopPropagation();
                showSideBox();
            });
            document.documentElement.appendChild(sideBoxRestoreButton);
        }

        const rect = video.getBoundingClientRect();
        const left = clamp(Math.round(rect.left + 12), 8, Math.max(8, window.innerWidth - 90));
        const top = clamp(Math.round(rect.bottom - 42), 8, Math.max(8, window.innerHeight - 38));
        sideBoxRestoreButton.style.left = `${left}px`;
        sideBoxRestoreButton.style.top = `${top}px`;
    }
    ...
    function createDebugPanel() {
        ...
        const steps = [
            ...
            ['8. Hide aria-label=Video player', debugHideVideoPlayer],
    ...
    function debugRunFullSequence() {
        ...
        if (debugInfoElement && debugInfoElement.parentElement === sideBoxInfo) {
            debugHideVideoPlayer();
        } else {
            debugLog('skip hiding aria-label=Video player because info was not moved');
        }
        updatePanel();
